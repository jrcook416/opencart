...
class ExportImport extends \Opencart\System\Engine\Controller {
	public function index(): void {
		...
		$data['save'] = $this->url->link('extension/export_import/other/export_import|save', 'user_token=' . $this->session->data['user_token']);
	...
	public function install(): void {
		...
		if (version_compare(VERSION,'4.0.1.0','>=')) {
			$data = [
				...
				'action'      => 'extension/export_import/other/export_import|eventViewCommonColumnLeftBefore',
				'status'      => true,
				'sort_order'  => 0
			];
			$this->model_setting_event->addEvent($data);
		} else {
			$this->model_setting_event->addEvent('other_export_import','','admin/view/common/column_left/before','extension/export_import/other/export_import|eventViewCommonColumnLeftBefore');
		}
